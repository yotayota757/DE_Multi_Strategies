...
def cone(individual, position, height, width):
    ...
    for x, p in zip(individual, position):
        value += (x - p)**2
    return height - width * np.sqrt(value)
    ...
    *bfunc* can also be given to act as static landscape (the default is no
    ...
    def changePeaks(self):
        ...
        for i in range(len(self.peaks_function)):
            ...
            shift_length = self.move_severity / np.sqrt(shift_length) if shift_length > 0 else 0
...
def diversity(population):
    ...
    return np.sqrt(sum((di - xi)**2 for x in population for di, xi in zip(d, x)))

if __name__ == "__main__":
    # mpb = MovingPeaks(dim=2, npeaks=[1,1,10], number_severity=0.1)
    mpb = MovingPeaks(dim=2, npeaks= 2, number_severity=0.1)
    # print(mpb.maximums())
    # mpb.changePeaks()
    # print(mpb.maximums())
    print(mpb.__call__([0, 0], count=False)[0])
    print(mpb.__call__([0, 1], count=False)[0])
    print(mpb.__call__([0, 2], count=False)[0])
    print(mpb.__call__([0, 3], count=False)[0])